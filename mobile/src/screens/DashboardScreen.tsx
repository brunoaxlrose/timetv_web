...
type DiscoverySection = 'populares' | 'em_breve' | 'em_curso' | 'top_10_filmes' | 'top_10_series';

const DASHBOARD_CACHE_TTL = 60_000;
let dashboardCache: Dados | null = null;
let dashboardCacheAt = 0;

export function DashboardScreen({ onOpenItem, onOpenDiscovery, refreshKey = 0 }: { onOpenItem: (item: Item) => void; onOpenDiscovery: (section: DiscoverySection) => void; refreshKey?: number }) {
      ...
      {loading ? (
        ...
        <>
          <View style={styles.sectionHeader}>
            <Pressable onPress={() => setCourseOpen((value) => !value)} style={styles.courseTitle}>
              <Text style={styles.sectionTitle}>Em curso</Text>
              <Text style={styles.count}>{data?.continuar_assistindo.length || 0}</Text>
              <Text style={styles.chevron}>{courseOpen ? '⌃' : '⌄'}</Text>
            </Pressable>
            <Pressable onPress={() => onOpenDiscovery('em_curso')} hitSlop={12}>
          ...
          {courseOpen ? (
            <>
              <View style={styles.switcher}>
                <Pressable onPress={() => setAba('continuar')} style={[styles.switchButton, aba === 'continuar' && styles.switchActive]}>
                  <Text style={[styles.switchText, aba === 'continuar' && styles.switchTextActive]}>Continuar</Text>
                </Pressable>
                <Pressable onPress={() => setAba('proximos')} style={[styles.switchButton, aba === 'proximos' && styles.switchActive]}>
                  <Text style={[styles.switchText, aba === 'proximos' && styles.switchTextActive]}>Proximos</Text>
                </Pressable>
              </View>
              {aba === 'continuar' ? (
                <FlatList
                  horizontal
                  data={data?.continuar_assistindo || []}
                  keyExtractor={(row) => String(row.next_episode.id_episodio)}
                  renderItem={({ item: row }) => <ContinueCard row={row} onPress={() => onOpenItem(row.item)} />}
                  showsHorizontalScrollIndicator={false}
                />
              ) : (
                <FlatList
                  horizontal
                  data={(data?.proximos || []).slice(0, 10)}
                  keyExtractor={(evento, index) => `${evento.id_item}-${evento.id_episodio || index}`}
                  renderItem={({ item: evento }) => <NextCard evento={evento} onPress={() => onOpenItem(evento as unknown as Item)} />}
                  showsHorizontalScrollIndicator={false}
                />
              )}
              {aba === 'continuar' && !data?.continuar_assistindo.length ? <Empty text="Nada para continuar por enquanto." /> : null}
              {aba === 'proximos' && !data?.proximos.length ? <Empty text="Nenhum proximo lancamento cadastrado." /> : null}
            </>
          ) : null}
...
function ContinueCard({ row, onPress }: { row: Dados['continuar_assistindo'][number]; onPress: () => void }) {
  ...
  return (
    <Pressable onPress={onPress} style={styles.trackingCard}>
      {row.item.url_poster ? <Image source={{ uri: row.item.url_poster }} style={styles.trackingPoster} /> : <View style={styles.trackingPoster} />}
      <View style={styles.trackingCopy}>
        <Text numberOfLines={1} style={styles.trackingTitle}>{row.item.titulo}</Text>
        <Text style={styles.episode}>T{String(row.next_episode.numero_temporada).padStart(2, '0')} · E{String(row.next_episode.numero_episodio).padStart(2, '0')}</Text>
        <Text numberOfLines={1} style={styles.episodeTitle}>{row.next_episode.titulo}</Text>
        <View style={styles.progressTrack}>
          <View style={[styles.progressFill, { width: `${percent}%` }]} />
        </View>
        <Text style={styles.progressMeta}>{watched}/{total || '?'} episodios{total ? ` · Restam ${remaining}` : ''}</Text>
      </View>
...
function NextCard({ evento, onPress }: { evento: EventoCalendario; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={styles.trackingCard}>
      {evento.url_poster ? <Image source={{ uri: evento.url_poster }} style={styles.trackingPoster} /> : <View style={styles.trackingPoster} />}
      <View style={styles.trackingCopy}>
        <Text numberOfLines={1} style={styles.trackingTitle}>{evento.titulo}</Text>
        <Text style={styles.episode}>{evento.numero_temporada != null ? `T${evento.numero_temporada} · E${evento.numero_episodio}` : 'ESTREIA'}</Text>
        <Text numberOfLines={1} style={styles.episodeTitle}>{evento.titulo_episodio || 'Novo lancamento'}</Text>
        <Text style={styles.date}>{new Date(`${evento.data_evento}T12:00:00`).toLocaleDateString('pt-BR', { weekday: 'short', day: '2-digit', month: 'short' })}</Text>
      </View>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  switcher: { backgroundColor: colors.surface, borderRadius: 15, flexDirection: 'row', marginBottom: 14, padding: 4 },
  switchButton: { alignItems: 'center', borderRadius: 11, flex: 1, paddingVertical: 11 },
  switchActive: { backgroundColor: colors.surfaceRaised },
  ...
  trackingCard: { backgroundColor: colors.surface, borderLeftColor: colors.accent, borderLeftWidth: 4, borderRadius: 15, flexDirection: 'row', marginBottom: 8, marginRight: 12, minHeight: 126, padding: 10, width: 310 },
  trackingPoster: { backgroundColor: colors.surface, borderRadius: 9, height: 106, width: 74 },
  trackingCopy: { flex: 1, paddingHorizontal: 12 },
  trackingTitle: { color: colors.text, fontSize: 17, fontWeight: '900' },
  episode: { color: colors.accent, fontWeight: '800', marginTop: 4 },
  episodeTitle: { color: colors.text, marginTop: 5 },
  progressTrack: { backgroundColor: colors.surfaceRaised, borderRadius: 4, height: 5, marginTop: 13, overflow: 'hidden' },
  progressFill: { backgroundColor: colors.accent, height: '100%' },
  progressMeta: { color: colors.muted, fontSize: 11, marginTop: 5, textAlign: 'right' },
